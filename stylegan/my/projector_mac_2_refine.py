...
"""Project given image to the latent space of pretrained network pickle using W+ latent space."""
...
def run_projection(network_pkl, target_fname, outdir, save_video, seed, num_steps):
    ...
    if network_pkl.endswith('.pt'):
        G = torch.load(network_pkl, map_location=device).eval().requires_grad_(False)
    else:
