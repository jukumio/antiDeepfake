...
def get_id_emb(id_net, id_img_path):
    ...
    if id_img is None:
        raise FileNotFoundError(f"Error: Unable to read image at {id_img_path}")

    id_img = cv2.resize(id_img, (112, 112))
    id_img = cv2paddle(id_img)
    
    if id_img is None or id_img.shape[1:] != (3, 112, 112):
        raise ValueError("Error: Image transformation failed, check preprocessing.")
...
def image_test(args):
    ...
    for img_path in img_list:
        ...
        if att_img is None:
            raise ValueError(f"Error: Face-aligned image {base_path}_aligned.png could not be loaded.")
        
        att_img = cv2paddle(att_img)
        res, mask = faceswap_model(att_img)
        
        if res is None or res.shape[1:] != (3, args.image_size, args.image_size):
            raise RuntimeError("Face Swap failed: Transformation is not possible.")
...
def face_align(landmarkModel, image_path, merge_result=False, image_size=224):
    ...
    for path in img_list:
        ...
        if landmark is None:
            raise ValueError(f"Error: No face landmarks detected in {path}. Face alignment failed.")
