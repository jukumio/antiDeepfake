...
class Solver(object):
    ...
    def pgd_attack_on_image(self, image_path, save_path):
        """제공된 이미지에 PGD 공격을 수행하여 변형"""
        ...
        x_real = transform(image).unsqueeze(0).to(self.device)  # 배치 차원 추가

        # 대상 속성(변경할 속성) 설정
        c_trg = torch.zeros((1, self.c_dim)).to(self.device)  # 예: 블론드 헤어로 변경
        c_trg[:, 1] = 1  # Blond Hair 속성 활성화

        # PGD 공격 수행
        pgd_attack = attacks.LinfPGDAttack(model=self.G, device=self.device, epsilon=0.05, k=10, a=0.01)
        x_adv, perturb = pgd_attack.perturb(x_real, x_real, c_trg)

        # 결과 저장
        save_image(self.denorm(x_adv.cpu()), save_path)
        print(f"Saved adversarial image at: {save_path}")

        def pgd_blur_attack_on_image(self, image_path, save_path):
        """제공된 이미지에 PGD + 블러 방어 우회 공격을 수행"""
        ...
        image = Image.open(image_path).convert('RGB')
        x_real = transform(image).unsqueeze(0).to(self.device)

        c_trg = torch.zeros((1, self.c_dim)).to(self.device) 
        c_trg[:, 1] = 1  

        # PGD + Blur 방어 우회 공격
        pgd_attack = attacks.LinfPGDAttack(model=self.G, device=self.device, epsilon=0.05, k=10, a=0.01)
        x_adv_blur, _, blurred_img = pgd_attack.perturb_blur(x_real, x_real, c_trg)

        # 결과 저장
        save_image(self.denorm(x_adv_blur.cpu()), save_path)
        print(f"Saved adversarial + blur attack image at: {save_path}")
