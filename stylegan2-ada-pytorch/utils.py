...
def lab_attack(X_nat, c_trg, model, epsilon=0.05, iter = 100):
    ...
    for i in range(iter):
        ...
        with torch.no_grad():
            c_trg_fixed = c_trg[i % len(c_trg)]

            # 4D 또는 3D라면 squeeze() 적용하여 2D로 변환
            if c_trg_fixed.ndim > 2:
                c_trg_fixed = c_trg_fixed.squeeze()

            # 1D일 경우에도 [1, c_dim] 형태로 변환
            if c_trg_fixed.ndim == 1:
                c_trg_fixed = c_trg_fixed.view(1, -1)

            gen_noattack, gen_feats_noattack = model(X_nat, c_trg_fixed)

        c_trg_fixed = c_trg[i % 5]
        ...
        gen_stargan, gen_feats_stargan = model(X_new, c_trg_fixed)
